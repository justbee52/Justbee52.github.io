            ...
            <div class="control-panel">
                ...
                <div id="actionsTab" class="tab-content">
                    ...
                    <div class="actions">
                        ...
                        <button class="btn" id="buyBtn" disabled title="Buy property for 500-1500 gold">🏰 Buy Property</button>
    ...
    <script>
        ...
        function rollDice() {
            if (gameState.currentPlayerIndex === gameState.myPlayerIndex && gameState.gameStarted) {
                ...
                document.querySelector(`.territory[data-index="${newPosition}"]`).classList.add("highlighted");
                ...
                buyBtn.disabled = gameState.territoryOwners[newPosition] !== null || gameState.territories[newPosition].cost > currentPlayer.money;
                buildBtn.disabled = !canBuildSettlement();
                developBtn.disabled = !canDevelopProperty();
                tributeBtn.disabled = gameState.territoryOwners[newPosition] !== gameState.myPlayerIndex;
        ...
        // Spin action wheel
        function spinWheel() {
            if (gameState.currentPlayerIndex === gameState.myPlayerIndex && gameState.gameStarted && !gameState.wheelSpun) {
        ...
        function buyProperty() {
            ...
            if (gameState.territoryOwners[territoryIndex] === null && currentPlayer.money >= territory.cost) {
        ...
        function buildSettlement() {
            ...
            if (canBuildSettlement()) {
                ...
                if (currentPlayer.victoryPoints >= 6 && currentPlayer.victoryPoints < 12) {
                    currentPlayer.money += 300;
                    logMessage(`${currentPlayer.name} earned 300 gold bonus at 6 VP!`);
                }
                updateBoard();
                updateUI();
                logMessage(`${currentPlayer.name} built a settlement on ${gameState.territories[territoryIndex].name}!`);
            }
        }

        // Develop property
        function developProperty() {
            const currentPlayer = gameState.players[gameState.myPlayerIndex];
            const territoryIndex = currentPlayer.position;
            if (canDevelopProperty()) {
                ...
                updateBoard();
                updateUI();
                logMessage(`${currentPlayer.name} developed ${gameState.territories[territoryIndex].name} for 1 VP!`);
            }
        }
        ...
        function collectRent() {
            ...
            if (gameState.territoryOwners[territoryIndex] === gameState.myPlayerIndex) {
                const rent = gameState.settlements[territoryIndex] ? 400 : 200;
                currentPlayer.money += rent;
                updateUI();
                logMessage(`${currentPlayer.name} collected ${rent} gold rent from ${gameState.territories[territoryIndex].name}!`);
            }
        }
        ...
        function updatePlayerPositions() {
            ...
            gameState.settlements.forEach((settlement, index) => {
                if (settlement === 1) {
                    const territory = gameState.territories[index];
                    const marker = document.createElementNS("http://www.w3.org/2000/svg", "rect");
                    marker.setAttribute("x", territory.centerX - 8);
                    marker.setAttribute("y", territory.centerY - 8);
                    marker.setAttribute("width", 16);
                    marker.setAttribute("height", 16);
                    marker.setAttribute("class", `settlement-marker player${gameState.territoryOwners[index]}`);
        ...
        function updateBoard() {
            const rects = boardSvg.querySelectorAll(".territory");
            rects.forEach((rect, index) => {
                rect.classList.remove("owned", "player0", "player1", "player2", "player3", "player4", "player5");
                const owner = gameState.territoryOwners[index];
                if (owner !== null) {
                    rect.classList.add("owned", `player${owner}`);
                }
            });
            updatePlayerPositions();
        }
        ...
        function updateUI() {
            ...
            if (gameState.myPlayerIndex >= 0) {
                ...
                victoryBar.style.width = `${(myPlayer.victoryPoints / 12) * 100}%`;
                ...
                actionWheel.classList.toggle("disabled", gameState.wheelSpun || gameState.currentPlayerIndex !== gameState.myPlayerIndex);
                borrowBtn.disabled = myPlayer.loans >= 2;
                repayBtn.disabled = myPlayer.loans === 0;
                if (myPlayer.victoryPoints >= 12) {
                    logMessage(`${myPlayer.name} has won the game with 12 victory points!`);
                    ...
                    repayBtn.disabled = true;
                    endTurnBtn.disabled = true;
                    alert(`${myPlayer.name} conquers the board!`);
                }
            }
        }
